 ...
 * This file includes the function canAllPatientsBeSeen that returns whether all the patients can be covered
 * and populates a schedule matching doctors to patients.
*/
...
// Takes in two maps mapping doctors to their available hours and patients to their required time to be treated
// and populates a schedule mapping each doctor to the set of patients they need to see.
// Returns whether all the patients can be seen by the set of doctors.
...
bool canAllPatientsBeSeen(const Map<string, int>& doctors,
                          ...
                          Map<string, Set<string>>& schedule) {
    Map<string, int> docCopy = doctors;
    Map<string, int> patCopy = patients;
    return canAllPatientsBeSeenRec(docCopy, patCopy, schedule);
}

// Returns a schedule matching doctors with which patients they can see.
bool canAllPatientsBeSeenRec(Map<string, int>& doctors,
                             Map<string, int>& patients,
                             Map<string, Set<string>>& schedule) {
    // If all the patients are covered, return true.
    if (patients.isEmpty()) {
        return true;
    }
    else {
        // Take the next patient for consideration.
        string nextPatient = patients.firstKey();
        int patientHours = patients.get(nextPatient);
        Map<string, int> currPatients = patients;
        currPatients.remove(nextPatient);
        // Consider which doctor can fit the patient in their schedule.
        for (const string& doctor: doctors.keys()) {
            int docHours = doctors.get(doctor);
            // If the doctor can see the patient, adjust their schedule and availability accordingly.
            if (docHours >= patientHours) {
                doctors[doctor] -= patientHours;
                if (canAllPatientsBeSeenRec(doctors, currPatients, schedule)) {
                    schedule[doctor] += nextPatient;
                    return true;
                }
                // Add the hours back to the doctor then try a new patient.
                doctors[doctor] += patientHours;
            }
        }
    }
...
STUDENT_TEST("Multiple doctors can see multiple patients (2).") {
    Map<string, Set<string>> schedule;
    ...
    Map<string, Set<string>> expectedSchedule = {
        { "Dr. House", { "Patient A", "Patient D", "Patient E" } },
        { "AutoDoc",   { "Patient B", "Patient C", "Patient F" } },
    };
    EXPECT_EQUAL(schedule, expectedSchedule);
}

STUDENT_TEST("Agreement in total hours doesn't mean a schedule exists (3).") {
    Map<string, int> doctors = {
        { "Doctor A", 11 },
        { "Doctor B", 11 },
        { "Doctor C", 11 },
