 ...
 * CS106B Winter 2021
 * Assignment 8
 * This file includes functions that manipulate DNA sequences depicted in a linked list.
 */
...
Nucleotide* toStrand(const string& str) {
    ...
    int strandLength = str.length();
    Nucleotide* firstNode = dna;
    Nucleotide* prevNode = dna;
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    Nucleotide* start = dna;
    Nucleotide* firstDna = dna;
    Nucleotide* firstTarget = target;

    while (dna != nullptr) {
        // Success if target is empty.
        if (target == nullptr) {
            return start;
        }
        Nucleotide* dnaNext = dna->next;
        Nucleotide* targetNext = target->next;
        // Keep iterating if two strands match up.
        if (dna->value == target->value) {
            target = targetNext;
            dna = dnaNext;
        }
        // If not matching, restart search at nucleotide right after initial.
        else {
            start = firstDna->next;
            target = firstTarget;
            dna = firstDna->next;
            firstDna = firstDna->next;
        }
    }
    // Failure if entire string searched and target not completely traversed.
    if (target != nullptr) {
        return nullptr;
    }
    return start;
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    // Finds strand in dna.
    Nucleotide* currDna = findFirst(dna, target);

    // Can't splice if strand not found
    if (currDna == nullptr) {
        return false;
    }
    ...
    // Case 1: target found at front of strand.
    if (currDna == dna) {
        // Adjusts previous and next then deletes each nucleotide of target found in strand.
        while (currTarget != nullptr && currDna->next != nullptr) {
            Nucleotide* following = currDna->next;
            following->prev = nullptr;
            dna = following;
        ...
        // Sets dna pointer for removing partial strand, accounts for special case of removing all but one nucleotide.
        if ((currDna->next != nullptr && currTarget == nullptr) || (currDna->next == nullptr && currTarget == nullptr)) {
            dna = currDna;
        }

        // Special Case: removing entire strand;
        else if (currDna->next == nullptr && currTarget->next == nullptr) {
            delete currDna;
            dna = nullptr;
        }
    }

    // Case 2: target not at beginning of strand.
    else {
        Nucleotide* lastKept = currDna->prev;
        // Adjusts previous and next then deletes each nucleotide of target found in strand.
        while (currTarget != nullptr && currDna->next != nullptr) {
            Nucleotide* following = currDna->next;
            lastKept->next = following;
            following->prev = lastKept;
        ...
        // Special case: target at end of strand.
        if (currDna->next == nullptr) {
            lastKept->next = nullptr;
            delete currDna;
        }
    }
    return true;
...
STUDENT_TEST("deleteNucleotides cleans up a single nucleotide.") {
    Nucleotide* dna = new Nucleotide;
    dna->value = 'G';
    dna->prev = nullptr;
    dna->next = nullptr;
...
STUDENT_TEST("fromDNA handles double-character strings.") {
    Nucleotide* dna = new Nucleotide;
    dna->value = 'A';
    dna->prev = nullptr;
...
STUDENT_TEST("toStrand converts a longer sequence.") {
    string original = "AAATTTCCCGGG";
...
STUDENT_TEST("findFirst finds the first instance of a strand if it occurs many times.") {
    Nucleotide* dna    = toStrand("CCCCCCCCCCCCCCCCCCCCC");
    Nucleotide* target = toStrand("CCC");
...
STUDENT_TEST("spliceFirst works on a single nucleotide.") {
    Nucleotide* dna    = toStrand("C");
    Nucleotide* target = toStrand("C");
