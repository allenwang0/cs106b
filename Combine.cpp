 ...
 * CS 106B
 * Assignment 5
 * This file contains a function that combines several vectors into one sorted vector.
 */
#include "GUI/SimpleTest.h"
#include "Combine.h"
using namespace std;

// Takes two vectors and returns a sorted vector containing all the elements of both vectors.
Vector<DataPoint> merge(Vector<DataPoint>& left, Vector<DataPoint>& right, Vector<DataPoint> out);

// Takes in a vector of vectors of datapoints, and returns a single vector containing the sorted elements of all the interior vectors.
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    Vector<DataPoint> out;
    // Base case: if there are no vectors.
    if (sequences.size() < 1) {
        return {};
    }
    // Base case: if the vector of vectors has been broken down into a single sorted vector.
    if (sequences.size() == 1) {
        return sequences[0];
    }
    else {
        // Split and sort the vector of vectors recursively.
        ...
        Vector<DataPoint> leftVec = combine(left);
        Vector<DataPoint> rightVec = combine(right);
        out = merge(leftVec, rightVec, out);
    }
    return out;
}
...
Vector<DataPoint> merge(Vector<DataPoint>& left, Vector<DataPoint>& right, Vector<DataPoint> out) {
    int leftIndex = 0;
    int rightIndex = 0;
    // Compare the first elements of both vectors, add the smaller one and adjust indices accordingly.
    while (leftIndex < left.size() && rightIndex < right.size()) {
        DataPoint leftElem = left[leftIndex];
        DataPoint rightElem = right[rightIndex];
        if (leftElem.weight <= rightElem.weight) {
            out.add(leftElem);
            leftIndex++;
        }
        else {
    ...
    // Append the rest of the longer vector to output.
    if (leftIndex == left.size()) {
        return out += right.subList(rightIndex);
    }
    else if (rightIndex == right.size()) {
        return out += left.subList(leftIndex);
    }
    return out;
}
...
STUDENT_TEST("10,000 sequences with 10,000 total elements") {
    Vector<Vector<DataPoint>> vec;
    for (int i = 0; i < 10000; i++) vec.add({{ "", i }});
    auto merged = combine(vec);
    EXPECT_EQUAL(merged.size(), 10000);
    EXPECT_EQUAL(merged[0].weight, 0);
    EXPECT_EQUAL(merged[9999].weight, 9999);
}
