 ...
 * Allen Wang
 * CS106B Winter 2021
 * This file is the first part of Assignment 2, a function that takes in a terrain, sources, and height and returns which areas are flooded.
 */


// This function takes in a terrain Grid of heights (double), the location of sources (location), and the maximimum flooded height
// of the water and returns a boolean grid representing which regions are flooded (true) or not (false.)
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
                            const Vector<GridLocation>& sources,
                            double height) {
    int rows = terrain.numRows();
    int cols = terrain.numCols();
    Grid<bool> isFlooded(rows, cols);
    GridLocation location;
    Queue<GridLocation> toBeFlooded;
    for (int i = 0; i < sources.size(); i++) {
        if (terrain.get(sources[i]) <= height) {
             isFlooded.set(sources[i], true);
             toBeFlooded.enqueue(sources[i]);
        }
    }
    while (toBeFlooded.isEmpty() == false) {
        GridLocation currentLocation = toBeFlooded.dequeue();
        GridLocation l1 = {currentLocation.row+1, currentLocation.col};
        GridLocation l2 = {currentLocation.row-1, currentLocation.col};
        GridLocation l3 = {currentLocation.row, currentLocation.col+1};
        GridLocation l4 = {currentLocation.row, currentLocation.col-1};
        Vector<GridLocation> adjacentSquares = { l1, l2, l3, l4 };
        for (int i = 0; i < 4; i++) {
            if (terrain.inBounds(adjacentSquares[i]) && terrain.get(adjacentSquares[i]) <= height && isFlooded.get(adjacentSquares[i])
                == false) {
                isFlooded.set(adjacentSquares[i], true);
                toBeFlooded.enqueue(adjacentSquares[i]);
            }
        }
    }
    return isFlooded;
...
STUDENT_TEST("Water can't cross a horizontal levee.") {
    Grid<double> world = {
        { 0, 0, 0 },
        { 2, 2, 2 },
        { 0, 0, 0 }
    ...
    /* Only locations above the barrier should be under water. */
    Grid<bool> water = floodedRegionsIn(world, sources, 1.0);
    Grid<bool> expected = {
        {  true, true, true },
        {  false, false, false },
