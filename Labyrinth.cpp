 ...
 * CS106B Winter 2021
 * Assignment 8
 * This file includes a function that determines whether the string contains a path to freedom from the given maze.
 */
#include "Labyrinth.h"
...
// Checks if the string is a valid path out of the maze.
bool isPathToFreedom(MazeCell* start, const string& moves) {
    string path = moves;
    int pathLength = path.length();
    int numObjects = 0;
    // If pick up item at start
    ...
    for (int i = 0; i < pathLength; i++) {
        ...
        // Accounts for pickups.
        if (start->whatsHere != Item::NOTHING) {
            numObjects++;
        }
    }
    // Success if picked up all objects.
    if (numObjects >= 3) {
        return true;
    }
    return false;
