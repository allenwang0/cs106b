 ...
 * This file includes a function canBeMadeDisasterReady that returns whether a network of cities can be covered and
 * populates a list of cities to supply.
*/
...
// Takes in a map mapping cities to their neighbors, number of cities possible to cover, and populates a list of locations
// to supply to cover all the cities in the network. Uncovered cities is a parameter that stores which cities are yet to be covered.
// Returns whether it is possible to cover the network with numCities supplied.
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
                            ...
                            Set<string>& supplyLocations) {
    Set<string> uncoveredCities = {};
    // Creates list of uncovered cities.
...
bool canBeMadeDisasterReadyRec(const Map<string, Set<string>>& roadNetwork,
                               ...
                               Set<string>& uncoveredCities) {
    if (numCities < 0) {
        error("ERROR: NEGATIVE CITIES");
    }
    // If there are cities left we cannot cover, return false.
    if (numCities == 0 && !uncoveredCities.isEmpty()) {
        return false;
    }
    // If every city is covered, return true.
    if (uncoveredCities.isEmpty() || roadNetwork.isEmpty()) {
        return true;
    }
    else if (numCities > 0) {
        // Take the next uncovered city for consideration.
        string nextCity = uncoveredCities.first();
        // Creates set of possible ways to supply the next city.
        Set<string> nextCitySet = {nextCity};
        nextCitySet += roadNetwork.get(nextCity);
        // Consider each way to supply the city.
        for (const string& maybeCity: nextCitySet) {
            // Creates set of cities covered if that location is supplied.
            Set<string> maybeCitySet = {maybeCity};
            maybeCitySet += roadNetwork.get(maybeCity);
            // Marks set of cities as covered.
            Set<string> restCities = uncoveredCities - maybeCitySet;
            if (canBeMadeDisasterReadyRec(roadNetwork, numCities - 1, supplyLocations, restCities)) {
                supplyLocations += maybeCity;
                return true;
            }
        }
    }
...
STUDENT_TEST("Can solve ethane, regardless of ordering, and produces output") {
    ...
    do {
        ...
        Map<string, Set<string>> map = makeSymmetric({
            { cities[2], { cities[0], cities[1], cities[6] } },
            { cities[3], { cities[4], cities[5], cities[7] } }
        });
