 ...
 * CS106B Winter 2021
 * Assignment 7 Part 3
 * This file includes functions that implement a linear probing hash table.
 */
#include "LinearProbingHashTable.h"
#include "GUI/SimpleTest.h"
using namespace std;

// Creates an empty hash table and sets all the elements to empty.
...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    // Cannot insert duplicates or into a full table.
    if (contains(elem) || size() == numSlots) return false;
    int index = 0;
    // Jump to first non-filled slot.
    while (elems[(bucket + index) % numSlots].type == SlotType::FILLED) {
        index++;
    }
    // Insert element and adjust type.
    elems[(bucket + index) % numSlots].value = elem;
    elems[(bucket + index) % numSlots].type = SlotType::FILLED;
    numElems++;
    return true;
}

// Returns whether the element is present in the table
bool LinearProbingHashTable::contains(const string& elem) const {
    int bucket = mHashFn(elem);
    int numSlots = mHashFn.numSlots();
    int index = 0;
    // Starts at bucket and continues until it either finds the element or goes through the entire table.
    while (elems[(bucket + index) % numSlots].type != SlotType::EMPTY && index < numSlots) {
        if (elems[(bucket + index) % numSlots].value == elem && elems[(bucket + index) % numSlots].type == SlotType::FILLED) {
            return true;
        }
        index++;
    }
...
// Removes element from table by replacing it with a tombstone..
bool LinearProbingHashTable::remove(const string& elem) {
    int bucket = mHashFn(elem);
    int numSlots = mHashFn.numSlots();
    // Cannot remove from empty table or if element is not in table.
    if (size() == 0 || !contains(elem)) return false;
    int index = 0;
    // Finds element, removes, and changes type to tombstone.
    while (elems[(bucket + index) % numSlots].type != SlotType::EMPTY) {
        if (elems[(bucket + index) % numSlots].value == elem) {
            elems[(bucket + index) % numSlots].type = SlotType::TOMBSTONE;
            numElems--;
            return true;
        }
        index++;
