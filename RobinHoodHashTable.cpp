 ...
 * CS106B Winter 2021
 * Assignment 7 Part 4
 * This file includes functions that implement a robin hood hash table.
 */
...
// Initializes the table and sets initial distance.
RobinHoodHashTable::RobinHoodHashTable(HashFunction<string> hashFn) {
    const int kInitialSize = hashFn.numSlots();
    mHashFn = hashFn;
    numElems = 0;
    elems = new Slot[kInitialSize];
    for (int i = 0; i < kInitialSize; i++) {
        elems[i].distance = EMPTY_SLOT;
    }
}

// Clears table and deallocates memory.
...
// Inserts element into proper spot.
bool RobinHoodHashTable::insert(const string& elem) {
    int numSlots = mHashFn.numSlots();
    int home = mHashFn(elem);
    string elemCopy = elem;
    // Cannot insert duplicates or into full table.
    if (contains(elem) || size() == numSlots) return false;
    int distanceTravelled = 0;
    int distanceFromHome = 0;
    // Finds an empty slot.
    while (elems[(home + distanceTravelled) % numSlots].distance != EMPTY_SLOT) {
        // If there is an element that is closer to home than we've travelled, swap the two and continue search with new element.
        if (elems[(home + distanceTravelled) % numSlots].distance < distanceFromHome) {
            swap(elems[(home + distanceTravelled) % numSlots].element, elemCopy);
            // Keeps track of new element's distance value in distanceFromHome variable.
            swap(elems[(home + distanceTravelled) % numSlots].distance, distanceFromHome);
        }
        distanceTravelled++;
        distanceFromHome++;
    }
    // Once empty slot is reached insert current element and distance.
    elems[(home + distanceTravelled) % numSlots].element = elemCopy;
    elems[(home + distanceTravelled) % numSlots].distance = distanceFromHome;
    numElems++;
    return true;
}

// Checks if an element exists in the table by checking the proper spot first and continuing until there's a blank or you've moved more steps
// than the current slot's distance
bool RobinHoodHashTable::contains(const string& elem) const {
    int bucket = mHashFn(elem);
    int numSlots = mHashFn.numSlots();
    int index = 0;
    // Begin search through filled slots and stop if the current slot's distance is greater than the distance travelled.
    while (elems[(bucket + index) % numSlots].distance != EMPTY_SLOT && elems[(bucket + index) % numSlots].distance >= index) {
        if (index == numSlots) {
            break;
        }
        // Element found if bucket contains element and slot not empty.
        if (elems[(bucket + index) % numSlots].element == elem && elems[(bucket + index) % numSlots].distance != EMPTY_SLOT) {
            return true;
        }
        index++;
    }
...
// Removes element and shifts elements not home backwards.
bool RobinHoodHashTable::remove(const string& elem) {
    int bucket = mHashFn(elem);
    int numSlots = mHashFn.numSlots();
    // Cannot remove if empty table or element not present.
    if (size() == 0 || !contains(elem)) return false;
    int index = 0;
    // Searches for element starting at bucket and stops when current slot's distance is greater than distance travelled.
    while (elems[(bucket + index) % numSlots].distance != EMPTY_SLOT && elems[(bucket + index) % numSlots].distance >= index) {
        if (index == numSlots) {
            break;
        }
        // Removes element.
        if (elems[(bucket + index) % numSlots].element == elem) {
            elems[(bucket + index) % numSlots].distance = EMPTY_SLOT;
            numElems--;
            index++;
            // Backwards shift deletion.
            while (elems[(bucket + index) % numSlots].distance != EMPTY_SLOT && elems[(bucket + index) % numSlots].distance != 0) {
                elems[(bucket + index) % numSlots].distance -= 1;
                swap(elems[(bucket + index - 1) % numSlots], elems[(bucket + index) % numSlots]);
                index++;
            }
            return true;
        }
...
// Prints contents of table including elements and distances.
void RobinHoodHashTable::printDebugInfo() const {
    for (int i = 0; i < mHashFn.numSlots(); i++) {
        cout << elems[i] << endl;
    }
}
